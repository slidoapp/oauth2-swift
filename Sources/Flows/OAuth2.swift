...
open class OAuth2: OAuth2Base {
	...
	*/
	...
	open func tokenExchangeRequest() throws -> {
		
	}

	/**
	Exchange the subject's refresh token for audience client.
			...
			perform(request: post) { response in
				do {
					...
					**Explanation:**
					...
					rom Section 5.1 of [RFC6749] is used here to carry the requested token, which allows this token exchange protocol to use the
...
/// Provides URNs (Uniform Resource Name) for different token types
enum TokenType: String {
	/// OAuth 2.0 access token issued by the given authorization server
	case accessToken = "urn:ietf:params:oauth:token-type:access_token"
	
	/// OAuth 2.0 refresh token issued by the given authorization server.
	case refreshToken = "urn:ietf:params:oauth:token-type:refresh_token"
	
	/// ID Token as defined in [Section 2 of OpenID Connect Core](https://openid.net/specs/openid-connect-core-1_0.html#IDToken)
	case idToken = "urn:ietf:params:oauth:token-type:id_token"
	
	/// Base64url-encoded SAML 1.1 token
	case saml1Token = "urn:ietf:params:oauth:token-type:saml1"

	/// Base64url-encoded SAML 2.0 token
	case saml2Token = "urn:ietf:params:oauth:token-type:saml2"
	
	var urn: String { rawValue }
}
